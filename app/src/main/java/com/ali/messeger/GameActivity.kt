...
class GameActivity : AppCompatActivity() {
    ...
    private fun updatePoints() {
        ...
        player2Points.text = "Player Y Points: ${gameManager.player2Points}"
    }
    ...
    private fun showWinner(winningLine: WinningLine) {
        val (winningBoxes, background) = when (winningLine) {
            ...
            WinningLine.COLUMN_0 -> Pair(listOf(two,five,eight), R.drawable.vertital_line)
            WinningLine.COLUMN_1 -> Pair(listOf(one,four,seven), R.drawable.vertical_line)
    ...
    winningBoxes.forEach {
        box -> box.background = ContextCompat.getDrawable(GameActivity@ this, background)
        }
    }
}